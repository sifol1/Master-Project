...
If the dimension is lower than 10, it will use *birthday_small_fields.rs*, while using dimension 10 or higher will run *birthday_big_fields.rs*. The difference is in how they go through checking whether they are 3-to-1. birthday_small_fields go through the entire field, only picking random elements and keeping track of all the triplicates it has found. birthday_big_fields uses *T* tests; it picks *T* random elements, and keeps track of all the triplicates it has found.
